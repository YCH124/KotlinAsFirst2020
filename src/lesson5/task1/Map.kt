...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    ...
    var minimalPrice: Double? = null
    for ((shop, value) in stuff) {
        if (value.first == kind) {
            if (suitableShop == null) {
                suitableShop = shop
                minimalPrice = value.second
            } else {
                if (value.second < minimalPrice!!) {
                    minimalPrice = value.second
                    suitableShop = shop
                }
            }
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for ((key, value) in list.withIndex()) {
        if (numbersMap.containsKey(number - value))
            return Pair(numbersMap[number - value]!!, key)
        else numbersMap[value] = key
    }
    return Pair(-1, -1)
}
